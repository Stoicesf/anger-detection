...
Usage:
  python scripts/build_hard_negative.py
"""
...
CNEV = Path(r"E:\桌面\test\CNEV_Vocalizations\Core Set")
OUT = ROOT / "dataset" / "hard_negative"
SR = 16000
    ...
    for bucket, paths in src.items():
        ...
        for i, p in enumerate(paths):
            ...
            if bucket in ("loud_normal", "excited") and rng.random() < 0.8:
                ...
                manifest.append(
                    {
                        "path": str(dst2.relative_to(ROOT)),
                        "bucket": bucket,
                        "source": str(p),
                        "aug": f"gain_{g:.2f}",
                        "language": "zh",
            ...
            if rng.random() < 0.35:
                ...
                manifest.append(
                    {
                        "path": str(dst3.relative_to(ROOT)),
                        "bucket": bucket,
                        "source": str(p),
