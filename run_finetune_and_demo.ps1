...
& $py scripts\finetune_casia.py --device cpu --epochs 35 --test-speaker ZhaoZuoxiang
if ($LASTEXITCODE -ne 0) { throw "finetune failed" }

Write-Host "==> Launch demo on http://127.0.0.1:8503" -ForegroundColor Cyan
& $py -m streamlit run demo\realtime_emotion_demo.py --server.port 8503 --server.headless true
